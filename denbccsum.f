...
c     here the parameters of the problem are set, as well as the sampling of the region in energy
      eemin = -8/ttp-6*abs(ttx)/ttp     !lower bound for energy  ! everything in terms of the bcc hop
      eemax = 8/ttp+6*abs(ttx)/ttp !upper bound
...
c     assigning weighting to points depending on index
                  ...
                  den3p = den3p + wt*fact*dexp(-argp*argp/del2) !delta function plus arg
                  den3m = den3m + wt*fact*dexp(-argm*argm/del2) !delta function minus arg
               end do
            end do
         end do
