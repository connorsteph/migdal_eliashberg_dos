...
#plt.figure()
#plt.grid(True)
#plt.plot([x/D for x in const_domain_lam_2], const_vals_lam_2, '-.', label = 'const_dos_lam_2')
#plt.plot([x/D for x in const_domain_lam_1], const_vals_lam_1, '--', label = 'const_dos_lam_1')
#plt.plot([x/D for x in bcc_domain_lam_2], bcc_vals_lam_2, '-.', label = 'bcc_lam_2')
#plt.plot([x/D for x in bcc_domain_lam_1], bcc_vals_lam_1, '--', label = 'bcc_lam_1')
#plt.xlabel(r'$\frac{\omega_E}{D}$', fontsize=14)
#plt.ylabel(r'$\frac{T_C}{\omega_E}$', fontsize=14)
#plt.legend(loc='best')
#plt.title('Critical Temp. versus Char. Freq. ')
#plt.savefig('tc_vs_w_e_const_and_bcc.pdf', bbox_inches='tight', dpi = 300)
